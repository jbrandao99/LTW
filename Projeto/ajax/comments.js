...
function refresh() {
    ...
    request.open('get', '../ajax/db_sendMessage.php?' + encodeForAjax({ 'last_id': last_id, 'place_id': place_id }), true);
...
function sendMessage(event) {
    ...
    // Send message
    let request = new XMLHttpRequest();
    request.open('get', '../ajax/db_sendMessage.php?' + encodeForAjax({ 'last_id': last_id, 'username': username, 'text': message, 'place_id': place_id }), true);
    request.addEventListener('load', messagesReceived);
    request.send();
